...
int main() {
    ...
    do {
        ...
        for (size_t i = 0; i < vertices.size(); i++) {
            cout << "ОС-" << vertices[i] << (i < vertices.size() - 1 ? " => " : " => Почтамат");
        }
        cout << " | длина: " << current << " км" << endl;
    ...
    cout << "Наименьшая длина пути: " << minDist << " км" << endl;
    cout << "Кратчайшие пути: " << endl;
    for (const auto& route : bestRoutes) {
        cout << "Почтамат -> ";
        for (size_t i = 0; i < route.size(); i++) {
            cout << "ОС-" << route[i] << (i < route.size() - 1 ? " -> " : " -> Почтамат");}
        cout << endl;
    }
    return 0;
}
